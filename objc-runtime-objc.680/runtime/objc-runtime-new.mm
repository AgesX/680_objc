...
attachCategories(Class cls, category_list *cats, bool flush_caches)
{
    ...
    /*
     二维数组
     method_list_t = [[method_t,method_t,...], [method_t], [method_t,method_t,method_t],...]
     */
    ...
    while (i--) { // 倒序遍历，最先访问最后编译的分类。
        ...
        // 获取该分类的方法列表(因为假设isMeta=NO，所以获取的是实例方法列表)
    ...
    // 获取宿主类当中的rw(读写)数据，其中包含宿主类的方法列表信息
    auto rw = cls->data();
    ...
    /*
     rw代表类
     methods代表类的方法列表
     attachLists方法的含义是 将含有mcount个元素的mlists拼接到rw的methods上
     */
